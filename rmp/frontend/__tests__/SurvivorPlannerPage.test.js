...
const odds = { week: 1, games: [{ game_id: 1, official_line: null, live_line: { favorite_team_id: 1, spread: 7, details: 'BUF -7', provider: 'ESPN' } }] };
...
describe('Survivor season planner', () => {
  ...
  test('explains plan privacy, saves a future plan, and keeps official submission explicit', async () => {
    ...
    global.fetch = jest.fn((url, options = {}) => {
      if (url.endsWith('/weeks/1/odds')) return response(odds);
      if (url.includes('/odds')) return response({ week: 2, games: [] });
  ...
  test('makes an eliminated entry read-only', async () => {
    global.fetch = jest.fn((url) => url.includes('/odds') ? response(odds) : response({ ...base, entries: [{ ...base.entries[0], alive: false }] }));
    render(<SurvivorPlannerPage />);
  ...
  test('ranks available teams by implied win probability and greys used teams', async () => {
    ...
    global.fetch = jest.fn((url) => url.includes('/odds') ? response(odds) : response(used));
    render(<SurvivorPlannerPage />);

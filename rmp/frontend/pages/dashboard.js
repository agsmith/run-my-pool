    ...
    return (
      <div className="pool-card__stats" style={{
        ...
        <div className="pool-card__stat-grid" style={{
          ...
          {[
            [league.pool_type === 'pickem' ? 'Entries' : league.pool_type === 'squares' ? 'Your Squares' : 'Entries Remaining', league.pool_type === 'squares' ? `${stats.total_entries ?? 0}` : `${stats.entries_remaining ?? 0}/${stats.total_entries ?? stats.entries_remaining ?? 0}`],
            ...
            <button
              ...
              onClick={() => router.push(league.pool_type === 'pickem' ? `/pool/${league.id}/pickem` : league.pool_type === 'squares' ? `/pool/${league.id}/squares` : `/pool/${league.id}/entries`)}
              style={{
              fontSize: '1.25rem',
              fontWeight: '700',
              color: '#d7ff3f',

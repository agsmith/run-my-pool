...
function AdminTable({ tab, records, currentUser, updateAccount }) {
  ...
  return <div className="platform-admin-table-wrap"><table className="platform-admin-table"><thead><tr><th>User</th><th>Role</th><th>Status</th><th>Pools</th><th>Joined</th><th>Actions</th></tr></thead><tbody>{visible.map((account) => <tr key={account.id}><td><strong>{account.email}</strong></td><td><span className={`platform-admin-role platform-admin-role--${account.role.toLowerCase()}`}>{roleLabel(account.role)}</span></td><td><span className={`platform-admin-status ${account.is_active ? 'is-active' : 'is-locked'}`}>{account.is_active ? 'Active' : 'Locked'}</span></td><td>{account.pool_count ?? 0}</td><td>{account.created_at ? new Date(account.created_at).toLocaleDateString() : '—'}</td><td><div className="platform-admin-actions">{tab === 'users' && <><button type="button" onClick={() => updateAccount(account, 'email')}>Edit email</button><button type="button" onClick={() => updateAccount(account, 'status')}>{account.is_active ? 'Deactivate' : 'Reactivate'}</button><button type="button" className="is-danger" onClick={() => updateAccount(account, 'delete')}>Delete</button></>}<button type="button" disabled={account.id === currentUser?.id && account.role === 'SUPER_ADMIN'} onClick={() => updateAccount(account, 'super-admin')}>{account.role === 'SUPER_ADMIN' ? 'Revoke super admin' : 'Grant super admin'}</button></div></td></tr>)}</tbody></table></div>;
}

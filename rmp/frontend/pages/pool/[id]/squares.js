...
export default function SquaresPage() {
  ...
  return <ProtectedRoute><main className="product-page-shell squares-page">
    ...
    {board.permissions.is_admin && board.plan === 'free' && <aside className="squares-upgrade"><div><span>Free Squares board</span><strong>{board.claims.length} of {board.block_limit ?? 25} included blocks reserved</strong><p>Upgrade to Commish to open all 100 blocks, assign blocks for members, and calculate the pot per reservation.</p></div><Link href="/pricing?checkout=commissioner">Upgrade to Commish</Link></aside>}

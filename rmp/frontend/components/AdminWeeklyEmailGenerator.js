...
export default function AdminWeeklyEmailGenerator({ poolId }) {
  const [week, setWeek] = useState(1);
  const [notes, setNotes] = useState('Weekly winner: $60. Season: $300 first, $100 second, $50 third. Monday tiebreaker: closest to the combined total; if equally close, the lower prediction wins; remaining ties split the pot.');
  ...
  return <section aria-labelledby="weekly-email-title" style={{ marginBottom: '3rem' }}>
    <h4 id="weekly-email-title" style={{ color: '#2d3748', marginBottom: '1rem' }}>Generate Weekly Email</h4>
    <div style={{ backgroundColor: 'white', padding: '1.5rem', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
      <p style={{ color: '#4a5568', marginTop: 0 }}>Create a tailored Pick ’Em update with weekly picks, standings, game results, and remaining-game scenarios.</p>
      ...
      <label style={{ display: 'block', marginTop: '1rem' }}>Payout and tiebreaker notes<textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} style={{ display: 'block', width: '100%', padding: '0.65rem', marginTop: '0.4rem' }} /></label>

...
export function PoolWorkspaceNav({ poolId, poolName, poolType = 'survivor', active, showAdmin = false }) {
  ...
  const items = [
    ...
    ...(poolType === 'survivor' ? [{ id: 'planner', label: 'Season Planner', href: `/pool/${poolId}/planner` }] : []),
    ...(poolType === 'pickem' ? [{ id: 'breakdown', label: 'Weekly Pick Breakdown', href: `/pool/${poolId}/breakdown` }] : []),
    ...(poolType === 'survivor' || poolType === 'pickem' ? [{ id: 'leaderboard', label: 'Season Leaderboard', href: `/pool/${poolId}/leaderboard` }] : []),

...
export default function PoolLaunchChecklist({ pool, onClose, onNavigate, onInviteCopied }) {
  ...
  return (
    <section className="pool-launch" aria-label="Pool launch checklist">
      ...
      <ol>
        ...
        <li className={completed.has('invite') ? 'is-complete' : ''}><span>{completed.has('invite') ? '✓' : '3'}</span><div><strong>Invite your players</strong><small>{pool.is_private ? 'Players will need both this link and your pool join code.' : 'Anyone with this link can find and join your public pool.'}</small><div className="pool-launch__invite"><input aria-label="Pool invitation link" value={inviteUrl} readOnly /><button type="button" onClick={copyInvite}>Copy invite link</button></div>{copyStatus && <small role="status">{copyStatus}</small>}</div></li>
        <li className={completed.has('entry') ? 'is-complete' : ''}><span>{completed.has('entry') ? '✓' : '4'}</span><div><strong>{pool.pool_type === 'pickem' ? 'Open your Pick ’Em board' : 'Create your first entry'}</strong><small>Experience the same starting point your players will use.</small><button type="button" onClick={() => completeAndNavigate('entry', pool.pool_type === 'pickem' ? `/pool/${pool.id}/pickem` : `/pool/${pool.id}/entries`)}>{pool.pool_type === 'pickem' ? 'Open Pick ’Em board' : 'Open My Entries'}</button></div></li>
      </ol>

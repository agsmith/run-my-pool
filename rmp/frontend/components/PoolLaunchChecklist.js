  ...
  return (
    <section className="pool-launch" aria-label="Pool launch checklist">
      ...
      <ol>
        ...
        <li className={completed.has('settings') ? 'is-complete' : ''}><span>{completed.has('settings') ? '✓' : '2'}</span><div><strong>Review commissioner settings</strong><small>{ownerManagedSquares ? 'Confirm the selected games, lock time, and payout setup.' : 'Confirm rules, access, and the join code before inviting players.'}</small><button type="button" onClick={() => completeAndNavigate('settings', `/admin/league/${pool.id}`)}>Review settings</button></div></li>
        {ownerManagedSquares ? <li className={completed.has('invite') ? 'is-complete' : ''}><span>{completed.has('invite') ? '✓' : '3'}</span><div><strong>Collect player selections yourself</strong><small>The free board is private to you. Receive names and block requests by text, email, or in person, then enter them on the board. Players cannot join online.</small><button type="button" onClick={() => completeAndNavigate('invite', `/pool/${pool.id}/squares`)}>Enter received selections</button><button type="button" onClick={() => onNavigate('/pricing?checkout=squares-plus')}>Upgrade for online player access</button></div></li> : <li className={completed.has('invite') ? 'is-complete' : ''}><span>{completed.has('invite') ? '✓' : '3'}</span><div><strong>Invite your players</strong><small>{pool.is_private ? 'Players will need both this link and your pool join code.' : 'Anyone with this link can find and join your public pool.'}</small><div className="pool-launch__invite"><input aria-label="Pool invitation link" value={inviteUrl} readOnly /><button type="button" onClick={copyInvite}>Copy invite link</button></div>{copyStatus && <small role="status">{copyStatus}</small>}<form className="pool-launch__email" onSubmit={sendInvite}><label htmlFor={`pool-invite-email-${pool.id}`}>Or send by email</label><div><input id={`pool-invite-email-${pool.id}`} type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="player@example.com" required /><button type="submit" disabled={sendingEmail}>{sendingEmail ? 'Sending…' : 'Send invite'}</button></div></form>{emailStatus && <small role="status">{emailStatus}</small>}</div></li>}
        <li className={completed.has('entry') ? 'is-complete' : ''}><span>{completed.has('entry') ? '✓' : '4'}</span><div><strong>{pool.pool_type === 'pickem' ? 'Open your Pick ’Em board' : pool.pool_type === 'squares' ? 'Open your Squares board' : 'Create your first entry'}</strong><small>{ownerManagedSquares ? 'Enter each player display name and reserve the blocks they requested.' : 'Experience the same starting point your players will use.'}</small><button type="button" onClick={() => completeAndNavigate('entry', pool.pool_type === 'pickem' ? `/pool/${pool.id}/pickem` : pool.pool_type === 'squares' ? `/pool/${pool.id}/squares` : `/pool/${pool.id}/entries`)}>{pool.pool_type === 'pickem' ? 'Open Pick ’Em board' : pool.pool_type === 'squares' ? 'Open Squares board' : 'Open My Entries'}</button></div></li>
      </ol>

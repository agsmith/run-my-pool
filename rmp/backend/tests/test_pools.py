...
class TestPoolEndpoints:
    ...
    @patch("pools.send_pool_invitation_email")
    def test_non_admin_cannot_email_pool_invitation(self, send_invitation, client):
    ...
    def test_duplicate_max_length_name_still_returns_suggestions(self, client):
        ...
        response = client.post("/pools/create", json={"name": long_name}, headers=owner)
    ...
    def test_pool_discovery_requires_auth_and_shows_private_leagues(self, client):
        ...
        owner_ids = {pool["id"] for pool in client.get("/pools/", headers=owner).json()}
...
class TestPoolJoining:
    ...
    def test_owner_can_save_recurring_weekly_lock(self, client):
        ...
        pool = client.post(
            "/pools/create",
            json={"name": "Weekly Pool", "is_private": False},
            headers=owner,
        ).json()
    ...
    def test_private_password_is_hashed_and_never_returned(self, client, db_session):
        ...
        pool = (
            ...
            .filter(models.Pool.id == response.json()["id"])
            .first()
        )
        ...
        revealed = client.get(f"/pools/{pool.id}/join-password", headers=headers)
    ...
    def test_join_password_is_only_viewable_by_league_admins(self, client):
        ...
        pool = client.post(
            ...
            json={
                "name": "View Password",
                "is_private": True,
                "join_password": "sideline8",
            },
            headers=owner,
        ).json()
        outsider = _register(client, "view.password.outsider@example.com")

        denied = client.get(f"/pools/{pool['id']}/join-password", headers=outsider)

        assert denied.status_code == 403
        assert "password" not in denied.json()

    def test_legacy_password_becomes_viewable_after_admin_changes_it(
        self, client, db_session
    ):
        ...
        pool = client.post(
            ...
            json={
                ...
                "is_private": True,
                "join_password": "original8",
            },
            headers=owner,
        ).json()
        stored = (
            db_session.query(models.Pool).filter(models.Pool.id == pool["id"]).one()
        )
        stored.join_password_encrypted = None
        db_session.commit()

        unavailable = client.get(f"/pools/{pool['id']}/join-password", headers=owner)
        ...
        revealed = client.get(f"/pools/{pool['id']}/join-password", headers=owner)
        assert revealed.json() == {"available": True, "password": "replacement9"}

    def test_public_pool_joins_without_password_and_appears_in_my_pools(
        self, client, caplog
    ):
        caplog.set_level(logging.INFO, logger="runmypool.pools")
        owner = _register(client, "public.owner@example.com")
        pool = client.post(
            "/pools/create",
            json={"name": "Open Pool", "is_private": False},
            headers=owner,
        ).json()
        ...
        joined_event = next(
            ...
            if record.name == "runmypool.pools" and record.getMessage() == "pool_joined"
        )
        assert joined_event.pool_id == pool["id"]
        assert joined_event.is_private is False
        assert not hasattr(joined_event, "password")
    ...
    def test_owner_can_flip_public_private_and_must_set_password(
        self, client, db_session
    ):
        owner = _register(client, "flip.owner@example.com")
        pool = client.post(
            "/pools/create",
            json={"name": "Flip Pool", "is_private": False},
            headers=owner,
        ).json()
        ...
        stored = (
            db_session.query(models.Pool).filter(models.Pool.id == pool["id"]).first()
        )
        assert stored.is_private is False
        assert stored.join_password_hash is None

    def test_delegated_admin_can_change_access_non_admin_cannot(
        self, client, db_session
    ):
        ...
        pool = client.post(
            "/pools/create",
            json={"name": "Admin Pool", "is_private": False},
            headers=owner,
        ).json()
        ...
        assert denied.status_code == 403
        revealed = client.get(f"/pools/{pool['id']}/join-password", headers=admin)
        assert revealed.status_code == 200

...
def list_pool_messages(pool_id: str, skip: int = 0, limit: int = 100, 
    ...
    if not user_entries:
        raise HTTPException(status_code=403, detail="You must be a member of this pool to view messages")
    
    # Get messages with user information
    messages = db.query(models.MessageBoard, models.User).join(
        models.User, models.MessageBoard.user_id == models.User.id
    ).filter(
        models.MessageBoard.pool_id == pool_id
    ).order_by(desc(models.MessageBoard.created_at)).offset(skip).limit(limit).all()
...
def post_message(pool_id: str, message: schemas.MessageBoardCreate, 
    ...
    if not user_entries:
        raise HTTPException(status_code=403, detail="You must be a member of this pool to post messages")
    ...
    log_create_operation(
        ...
        entity_data={
            "pool_id": pool_id,
            "message": message.message.strip()[:100] + "..." if len(message.message.strip()) > 100 else message.message.strip(),
...
def delete_message(message_id: str, 
    ...
    if not user_entries:
        raise HTTPException(status_code=403, detail="You no longer have access to this pool")
